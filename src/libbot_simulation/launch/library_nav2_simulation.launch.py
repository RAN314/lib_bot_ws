#!/usr/bin/env python3
# 图书馆机器人Nav2导航仿真启动文件
# 基于 nav2_bringup/tb3_simulation_launch.py 修改
...
def generate_launch_description():
    ...
    declare_robot_sdf_cmd = DeclareLaunchArgument(
        'robot_sdf',
        default_value=os.path.join(pkg_turtlebot3_gazebo, 'models', 'turtlebot3_waffle_pi', 'model.sdf'),
        description='Full path to robot sdf file to spawn the robot in gazebo')

    # 设置 Gazebo 模型路径环境变量 - 追加到现有路径
    gazebo_model_path_bookstore = AppendEnvironmentVariable(
        'GAZEBO_MODEL_PATH',
        os.path.join(pkg_bookstore_world, 'models'))

    gazebo_model_path_turtlebot3 = AppendEnvironmentVariable(
        'GAZEBO_MODEL_PATH',
    ...
    # 使用 turtlebot3_gazebo 的 SDF 模型中的 URDF 用于 robot_state_publisher
    # 注意：需要从 SDF 中提取 URDF 或使用 description 包中的 URDF
    ...
    # 在 Gazebo 中生成机器人（使用 SDF 文件）
    # SDF 文件包含 Gazebo 插件（差分驱动、激光雷达等）
    ...
    # 设置环境变量
    ld.add_action(gazebo_model_path_bookstore)
    ld.add_action(gazebo_model_path_turtlebot3)
